...
public interface ExpenseSplitRepository extends JpaRepository<ExpenseSplit, UUID> {
    ...
    @Query(value = """
        ...
        FROM
          (
            SELECT DISTINCT user_id FROM (
              ...
              UNION
              SELECT debtor_id AS user_id FROM settlements WHERE group_id = :groupId
              UNION
              SELECT creditor_id AS user_id FROM settlements WHERE group_id = :groupId
            ) all_users
          ) u
        LEFT JOIN (
        ...
        LEFT JOIN (
            SELECT debtor_id, SUM(amount) AS settlement_paid
              FROM settlements
             WHERE group_id = :groupId
             GROUP BY debtor_id
        ) settled_paid ON settled_paid.debtor_id = u.user_id
        LEFT JOIN (
            SELECT creditor_id, SUM(amount) AS settlement_received
              FROM settlements
             WHERE group_id = :groupId
             GROUP BY creditor_id
        ) settled_received ON settled_received.creditor_id = u.user_id
